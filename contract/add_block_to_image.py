...
if __name__ == '__main__':
    ...
    save_path = os.getcwd() + "/contract/nho_doi_at_thanh_marker_result_06042023/"
    ...
    while img_cnt < 500:
        try:
            ...
            order_type = np.random.choice(['up|down', 'left|right'], p=[0.7, 0.3])
            if order_type == 'up|down':
                partyA = Party(skip_prob = 0.1, down_prob = 0.6, bank_prob = 0.6, font_size=font_size)()
                partyB = Party(skip_prob = 0.1, down_prob = 0.6, bank_prob = 0.6, font_size=font_size)()
            else:
